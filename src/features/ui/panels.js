...
function applyToolbarAccess() {
  ...
  document.querySelectorAll(".tool[data-tool]").forEach((button) => {
    ...
    if (TOOL_ICONS[tool] && !button.querySelector(".tool-icon")) button.insertAdjacentHTML("afterbegin", `<span class="tool-icon">${TOOL_ICONS[tool]}</span>`);
  });
  const rules = {
  ...
  for (const [id, permission] of Object.entries(rules)) {
    ...
    if (node) node.hidden = !can(access, permission);
  }
  ["regionsToggle"].forEach((id) => { const node = document.getElementById(id); if (node) node.hidden = !can(access, "regions.view"); });
  ["zoomInButton", "zoomOutButton"].forEach((id) => { const node = document.getElementById(id); if (node) node.hidden = !can(access, "map.zoom"); });
  ["resetMapButton"].forEach((id) => { const node = document.getElementById(id); if (node) node.hidden = !can(access, "map.reset"); });
  ["mapLayerButton", "satelliteLayerButton"].forEach((id) => { const node = document.getElementById(id); if (node) node.hidden = !can(access, "map.layer"); });
}
...
export function bindPanels(elements, mapState, drawing, handlers) {
  ...
  document.addEventListener("click", async (event) => {
    ...
    if (!exportButton || exportButton.hidden || !can(window.RegionConsoleRBAC?.access, "data.export")) return;
    event.preventDefault(); event.stopImmediatePropagation(); exportButton.classList.add("active");
    try { const count = await exportToFolders(); window.dispatchEvent(new CustomEvent("region-console:toast", { detail: { message: `${count} alan klasör yapısına kaydedildi.` } })); }
    catch (error) { if (error?.name !== "AbortError") { console.error("[Region Console] Folder export failed:", error); window.alert(`Klasör kaydı başarısız: ${error.message || "Bilinmeyen hata"}`); } }
    finally { exportButton.classList.remove("active"); }
  }, true);

  document.addEventListener("click", (event) => {
    const historyButton = event.target?.closest?.('.tool[data-tool="history"]');
    if (!historyButton || historyButton.hidden || !can(window.RegionConsoleRBAC?.access, "history.view")) return;
    event.preventDefault(); event.stopImmediatePropagation(); document.querySelectorAll(".tool:not(.tool-action)").forEach((button) => button.classList.remove("active")); historyButton.classList.add("active"); showHistory(elements);
  }, true);
  ...
  document.querySelectorAll(".tool:not(.tool-action)").forEach((button) => {
    ...
    button.addEventListener("click", () => {
      const tool = button.dataset.tool;
      if (!can(window.RegionConsoleRBAC?.access, TOOL_PERMISSIONS[tool])) return;
      document.querySelectorAll(".tool:not(.tool-action)").forEach((b) => b.classList.remove("active")); button.classList.add("active"); handlers.onTool?.(tool);
    });
  });
  ...
  elements.addRegionButton?.addEventListener("click", (event) => {
    if (!can(window.RegionConsoleRBAC?.access, "regions.create")) return;
    event.preventDefault(); event.stopPropagation(); elements.sidebar.hidden = true; elements.regionsToggle?.setAttribute("aria-expanded", "false"); document.querySelectorAll(".tool:not(.tool-action)").forEach((button) => button.classList.toggle("active", button.dataset.tool === "draw")); handlers.onTool?.("draw");
  });
  ...
  document.getElementById("themeButton")?.addEventListener("click", () => { if (can(window.RegionConsoleRBAC?.access, "map.view")) handlers.onTheme?.(); });
