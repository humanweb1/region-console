...
export function renderRegionsOnMap(mapState, regions = [], settings = null) {
  ...
  for (const region of regions) {
    ...
    const outside = region.status === "outside"; const closed = region.status === "closed"; const campaign = isCampaignRegion(region); const category = regionCategory(region); const isService = !outside && !closed && !campaign; const fillColor = outside ? normalized.outsideColor : closed ? normalized.closedColor : campaign ? normalized.campaignColor : normalized.boundaryColor; const fillOpacity = outside ? normalized.outsideOpacity : closed ? normalized.closedOpacity : campaign ? normalized.campaignOpacity : 0.72;
    ...
    polygon.options._baseFillOpacity = fillOpacity; polygon._regionLayerKind = category; polygon._regionCategory = category; polygon._regionStatus = outside ? "outside" : closed ? "closed" : campaign ? "campaign" : isService ? "service" : "service"; mapState.regionLayers.push(polygon); if (mapState.overlayVisibility[category] !== false) polygon.addTo(mapState.polygons);
