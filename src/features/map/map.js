...
function regionCategory(region) { const type = String(region?.hierarchy?.type || region?.type || "").trim().toLowerCase(); if (["province", "provinces", "il"].includes(type)) return "province"; if (["district", "districts", "ilce", "ilçe"].includes(type)) return "district"; if (["neighborhood", "neighbourhood", "mahalle"].includes(type)) return "neighborhood"; if (["cemetery", "mezarlik", "mezarlık"].includes(type)) return "cemetery"; return "special"; }
