...
function applyPermissions() {
  ...
  const toolPermissions = { draw:["service_areas.manage"], edit:["service_areas.manage","regions.manage"], delete:["service_areas.manage","regions.manage"], import:["service_areas.manage","regions.manage"], export:["data.export"], history:["history.view"] };
  document.querySelectorAll(".tool[data-tool]").forEach((button) => { const permissions = toolPermissions[button.dataset.tool]; if (permissions) hide(button, !any(...permissions)); });
  applied = true;
}
function permissionCards(selected = []) { const set = new Set(selected); return PERMISSIONS.map(([value,label]) => `<label class="rbac-permission"><input type="checkbox" name="permission" value="${value}" ${set.has(value) ? "checked" : ""}><span>${label}</span><code>${value}</code></label>`).join(""); }
function roleOptions(roles, selected) { return (roles || []).map((role) => `<option value="${escapeHtml(role.id)}" ${String(role.id) === String(selected) ? "selected" : ""}>${escapeHtml(role.name)} — ${escapeHtml(role.description || "")}</option>`).join(""); }
...
function renderManager(data) {
  ...
  elements.dialogBody.querySelectorAll("[data-rbac-tab]").forEach((button) => button.addEventListener("click", () => { const tab = button.dataset.rbacTab; elements.dialogBody.querySelectorAll("[data-rbac-section]").forEach((section) => { section.hidden = section.dataset.rbacSection !== tab; }); elements.dialogBody.querySelectorAll("[data-rbac-tab]").forEach((item) => item.classList.toggle("button-primary", item === button)); }));
  ...
  elements.dialogBody.querySelectorAll(".rbac-user-save").forEach((button) => button.addEventListener("click", async () => {
    const card = button.closest("[data-user-id]"), error = card.querySelector(".rbac-user-error"); button.disabled = true;
    ...
    catch (err) { notify(err.message || "Kullanıcı güncellenemedi."); if (error) error.textContent = err.message; }
    finally { button.disabled = false; }
  }));
  elements.dialogBody.querySelector("#createRoleForm")?.addEventListener("submit", async (event) => {
    event.preventDefault(); const form = event.currentTarget, error = form.querySelector("#createRoleError"), payload = Object.fromEntries(new FormData(form).entries()), button = form.querySelector("button[type=submit]"); error.textContent = ""; payload.permissions = [...form.querySelectorAll("input[name=permission]:checked")].map((input) => input.value); button.disabled = true;
    try { await createRole(session().access_token, payload); notify("Rol oluşturuldu."); renderManager(await listUsers(session().access_token)); } catch (err) { error.textContent = err.message; } finally { button.disabled = false; }
  });
  elements.dialogBody.querySelectorAll(".rbac-role-form").forEach((form) => form.addEventListener("submit", async (event) => {
    event.preventDefault(); const error = form.querySelector(".form-error"), payload = Object.fromEntries(new FormData(form).entries()), button = form.querySelector("button[type=submit]"); error.textContent = ""; payload.role_id = form.dataset.roleId; payload.permissions = [...form.querySelectorAll("input[name=permission]:checked")].map((input) => input.value); button.disabled = true;
    try { await updateRole(session().access_token, payload); notify("Rol güncellendi."); renderManager(await listUsers(session().access_token)); } catch (err) { error.textContent = err.message; } finally { button.disabled = false; }
  }));
}
function bindOverrides() {
  elements.usersButton?.addEventListener("click", (event) => { if (!allowed("users.manage")) { event.preventDefault(); event.stopImmediatePropagation(); return; } event.preventDefault(); event.stopImmediatePropagation(); renderUserManager(); }, true);
  const observer = new MutationObserver(() => { if (!access) return; const newCampaign = elements.dialogBody?.querySelector("#newCampaign"); if (newCampaign && !allowed("campaigns.manage")) newCampaign.remove(); });
  if (elements.dialogBody) observer.observe(elements.dialogBody, { childList:true, subtree:true });
}
async function refresh() { const currentSession = session(); if (!currentSession?.access_token || !currentSession?.user?.id) return; try { access = await getAccess(currentSession.access_token, currentSession.user.id); if (!access?.profile?.is_active) throw new Error("Kullanıcı hesabınız pasif."); applyPermissions(); } catch (error) { console.error("[Region Console] RBAC yüklenemedi:", error); access = null; applied = false; } }
store.subscribe(() => { const currentSession = session(); if (currentSession && !applied) refresh(); if (!currentSession) { access=null; applied=false; } });
bindOverrides(); if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", refresh, { once:true }); else refresh();
