...
function perms(selected = []) { const set = new Set(selected); return PERMISSIONS.map(([value, label]) => `<label class="rbac-permission"><input type="checkbox" name="permission" value="${esc(value)}" ${set.has(value) ? "checked" : ""}><span>${esc(label)}</span><code>${esc(value)}</code></label>`).join(""); }
...
async function ensureAccess(force = false) {
  ...
  const key = current?.access_token && current?.user?.id ? `${current.user.id}:${current.access_token}` : "";
  if (!key) { access = null; return null; }
  if (!force && key === lastSessionKey && access?.loaded) return access;
  if (loadingAccess) return access;
  loadingAccess = true;
  try { access = await getAccess(current.access_token, current.user.id); lastSessionKey = key; return access; }
  catch (error) { console.error("RBAC management", error); access = null; return null; }
  finally { loadingAccess = false; }
}

async function openManager() {
  access = window.RegionConsoleRBAC?.access || access;
  if (!access?.loaded) access = await ensureAccess(true);
...
function render(data) {
  ...
  const createRoleForm = dialog.body.querySelector("#createRoleForm"); wire(createRoleForm); createRoleForm.addEventListener("submit", (event) => { event.preventDefault(); saveCreateRole(createRoleForm); });
...
async function saveCreateRole(form) {
  const button = form.querySelector("button[type=submit]"); const error = form.querySelector(".form-error"); const payload = Object.fromEntries(new FormData(form).entries());
  ...
  try { await createRole(session().access_token, payload); toast({ toast: document.getElementById("toast") }, "Rol ve bölge kapsamı oluşturuldu."); window.dispatchEvent(new CustomEvent("region-console:rbac-refresh")); render(await listUsers(session().access_token)); }
  catch (exception) { error.textContent = exception.message; } finally { button.disabled = false; }
}
async function saveRole(form) {
  const button = form.querySelector("button[type=submit]"); const error = form.querySelector(".form-error"); const payload = Object.fromEntries(new FormData(form).entries());
  payload.role_id = form.dataset.roleId; payload.permissions = [...form.querySelectorAll("input[name=permission]:checked")].map((input) => input.value); payload.scopes = read(form); button.disabled = true; error.textContent = "";
  try { await updateRole(session().access_token, payload); toast({ toast: document.getElementById("toast") }, "Rol ve bölge yetkileri güncellendi."); window.dispatchEvent(new CustomEvent("region-console:rbac-refresh")); render(await listUsers(session().access_token)); }
  catch (exception) { error.textContent = exception.message; } finally { button.disabled = false; }
}
...
async function saveUser(saveButton) {
  ...
  try { await updateUser(session().access_token, { user_id: card.dataset.userId, role_id: card.querySelector(".rbac-user-role").value, is_active: card.querySelector(".rbac-user-active").checked }); toast({ toast: document.getElementById("toast") }, "Kullanıcı güncellendi."); }
  catch (exception) { toast({ toast: document.getElementById("toast") }, exception.message); } finally { saveButton.disabled = false; }
}

button?.addEventListener("click", (event) => { event.preventDefault(); event.stopImmediatePropagation(); openManager(); }, true);
store.subscribe(() => { const current = session(); const key = current?.access_token && current?.user?.id ? `${current.user.id}:${current.access_token}` : ""; if (key && key !== lastSessionKey) ensureAccess(false); });
