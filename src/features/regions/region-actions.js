...
function getRegion() { if (!selected?.region?.id) return null; return (store.get().regions.custom || []).find((item) => String(item.id) === String(selected.region.id)) || null; }
...
function showServiceDialog() {
  ...
  elements.dialogBody.querySelector("#serviceCloseConfirm").addEventListener("click", () => {
    ...
    commitRegion("Bölge hizmete kapatıldı", () => { const current = getRegion(); if (!current) return; store.update("regions", { custom: store.get().regions.custom.map((item) => item.id === current.id ? { ...current, status: "closed", serviceCloseReason: reason, serviceClosedAt: new Date().toISOString(), updatedAt: new Date().toISOString() } : item) }); });
...
function geometryToLatLngs(geometry) { if (!geometry || geometry.type !== "Polygon") return []; const ring = (geometry.coordinates?.[0] || []).map(([lng, lat]) => L.latLng(lat, lng)); if (ring.length > 1 && ring[0].equals(ring[ring.length - 1])) ring.pop(); return ring; }
function geometryToEditParts(geometry) { if (!geometry) return []; if (geometry.type === "Polygon") return [geometryToLatLngs(geometry)].filter((points) => points.length >= 3); if (geometry.type === "MultiPolygon") return (geometry.coordinates || []).map((polygon) => geometryToLatLngs({ type: "Polygon", coordinates: polygon })).filter((points) => points.length >= 3); return []; }
function ringToCoordinates(points) { const coordinates = points.map((point) => [point.lng, point.lat]); if (coordinates.length && (coordinates[0][0] !== coordinates.at(-1)[0] || coordinates[0][1] !== coordinates.at(-1)[1])) coordinates.push([...coordinates[0]]); return coordinates; }
function latLngsToGeometry(latLngs) { return { type: "Polygon", coordinates: [ringToCoordinates(latLngs)] }; }
function editPartsToGeometry(parts, originalGeometry) { const polygons = parts.map((part) => [ringToCoordinates(part.markers.map((marker) => marker.getLatLng()))]); if (originalGeometry?.type === "MultiPolygon") return { type: "MultiPolygon", coordinates: polygons }; return { type: "Polygon", coordinates: polygons[0] || [[]] }; }
function clearEditMarkers() { editMarkers.forEach((marker) => marker.remove()); editMidMarkers.forEach((marker) => marker.remove()); editParts.forEach((part) => { part.markers?.forEach((marker) => marker.remove()); part.midMarkers?.forEach((marker) => marker.remove()); part.layer?.remove(); }); editMarkers = []; editMidMarkers = []; editParts = []; }
function syncEditLayer() { if (editParts.length) { editParts.forEach((part) => part.layer.setLatLngs(part.markers.map((marker) => marker.getLatLng()))); return; } if (!editLayer) return; editLayer.setLatLngs(editMarkers.map((marker) => marker.getLatLng())); }
function vertexIcon(index) { return L.divIcon({ className: "boundary-vertex-marker-wrap", html: `<span class="boundary-vertex-marker" title="Nokta ${index + 1}">${index + 1}</span>`, iconSize: [24, 24], iconAnchor: [12, 12] }); }
function midpointIcon() { return L.divIcon({ className: "boundary-midpoint-marker-wrap", html: `<span class="boundary-midpoint-marker-wrap"><span class="boundary-midpoint-marker" title="Araya nokta ekle">+</span></span>`, iconSize: [18, 18], iconAnchor: [9, 9] }); }
function rebuildPartMidMarkers(part) { part.midMarkers.forEach((marker) => marker.remove()); part.midMarkers = []; const map = selected?.mapState?.map; const points = part.markers.map((marker) => marker.getLatLng()); if (!map || points.length < 3) return; points.forEach((point, index) => { const next = points[(index + 1) % points.length]; const midpoint = L.latLng((point.lat + next.lat) / 2, (point.lng + next.lng) / 2); const marker = L.marker(midpoint, { draggable: false, keyboard: false, zIndexOffset: 1100, icon: midpointIcon() }).addTo(map); marker.on("click", (event) => { L.DomEvent.stopPropagation(event); const nextPoints = part.markers.map((item) => item.getLatLng()); nextPoints.splice(index + 1, 0, marker.getLatLng()); rebuildPartMarkers(part, nextPoints); }); part.midMarkers.push(marker); }); }
function rebuildPartMarkers(part, points) { part.markers.forEach((marker) => marker.remove()); part.midMarkers.forEach((marker) => marker.remove()); part.markers = []; part.midMarkers = []; const map = selected?.mapState?.map; if (!map || points.length < 3) return; points.forEach((point, index) => { const marker = L.marker(point, { draggable: true, autoPan: true, keyboard: false, zIndexOffset: 1200, icon: vertexIcon(index) }).addTo(map); marker.on("dragstart", () => rebuildPartMidMarkers(part)); marker.on("drag", syncEditLayer); marker.on("dragend", () => { syncEditLayer(); rebuildPartMidMarkers(part); }); marker.on("dblclick", (event) => { L.DomEvent.stopPropagation(event); const currentPoints = part.markers.map((item) => item.getLatLng()); if (currentPoints.length <= 3) return toast(elements, "Sınır için en az 3 nokta gerekir."); currentPoints.splice(index, 1); rebuildPartMarkers(part, currentPoints); syncEditLayer(); }); part.markers.push(marker); }); rebuildPartMidMarkers(part); }
