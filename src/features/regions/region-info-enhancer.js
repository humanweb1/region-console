...
function candidates() {
  ...
  const walk = (items) => {
    for (const item of items || []) {
      ...
      walk(item.provinces);
      walk(item.districts);
      walk(item.neighborhoods);
      walk(item.cemeteries);
      walk(item.children);
    }
  };
...
function hierarchyPath(region, all) {
  ...
  while (current && !visited.has(current)) {
    visited.add(current);
    if (current.name) chain.unshift(String(current.name));
...
function activeCampaign(region) {
  if (!region?.campaignId) return null;
  return (store.get().campaigns || []).find((campaign) => String(campaign.id) === String(region.campaignId)) || null;
}
...
function finishCampaign(region) {
  ...
  const next = {
    ...
    campaignId: null,
    campaign: false,
    status: current.status === "campaign" ? "service" : current.status,
    updatedAt: new Date().toISOString()
  };
  store.update("regions", {
    custom: store.get().regions.custom.map((item) => String(item.id) === String(current.id) ? next : item)
  });
...
function renderInfo(panel, region) {
  ...
  info.innerHTML = `
    <div class="region-info-title">Bölge Durumu</div>
    <div class="region-info-grid">
      <div><span>Durum</span><strong class="region-status-value ${statusClass}">${escapeHtml(statusLabel)}</strong></div>
      <div><span>Bölge Tipi</span><strong>${escapeHtml(TYPE_LABELS[type] || type)}</strong></div>
      <div class="region-info-wide"><span>Hiyerarşi</span><strong>${escapeHtml(path)}</strong></div>
      ${region.status === "closed" && region.serviceCloseReason ? `<div class="region-info-wide"><span>Hizmete Kapalı Nedeni</span><strong>${escapeHtml(region.serviceCloseReason)}</strong></div>` : ""}
      ${campaign ? `<div class="region-info-wide"><span>Aktif Kampanya</span><strong>${escapeHtml(campaign.name)}</strong></div>` : ""}
    </div>
    ${campaign ? `<button id="regionCampaignEndButton" class="button region-campaign-end-button" type="button">Kampanyayı sonlandır</button>` : ""}
  `;
  ...
  info.querySelector("#regionCampaignEndButton")?.addEventListener("click", () => {
    ...
    if (fresh) renderInfo(panel, fresh);
  });
}
...
function enhancePanel() {
  ...
  if (!region) return;
  renderInfo(panel, region);
}

const style = document.createElement("style");
style.textContent = `
.region-info-block{margin:10px 0 12px;padding:12px;border:1px solid rgba(148,163,184,.2);border-radius:10px;background:rgba(15,23,42,.42)}
.region-info-title{font-weight:700;font-size:13px;margin-bottom:10px}
.region-info-grid{display:grid;grid-template-columns:1fr 1fr;gap:9px}
.region-info-grid>div{display:grid;gap:3px;min-width:0}.region-info-grid span{font-size:11px;opacity:.6}.region-info-grid strong{font-size:12px;overflow-wrap:anywhere}.region-info-wide{grid-column:1/-1}
.region-status-value.service{color:#34d399}.region-status-value.closed{color:#a78bfa}.region-status-value.outside{color:#94a3b8}.region-status-value.campaign{color:#facc15}
.region-campaign-end-button{width:100%;margin-top:10px}
@media(max-width:600px){.region-info-grid{grid-template-columns:1fr}.region-info-wide{grid-column:auto}}
`;
