...
function finishCleanup() {
  ...
  mapState.map.off("click", onMapClick);
  mapState.map.off("dblclick", onDoubleClick);
  if (previousDoubleClickZoom) mapState.map.doubleClickZoom.enable(); else mapState.map.doubleClickZoom.disable();
  line?.remove(); line = null; markers?.clearLayers();
  mode = null; context = null; points = [];
}
function showPreview() {
  ...
  if (line) line.remove();
  if (points.length >= 2) line = window.L.polyline(points, { weight: 3, dashArray: "6 5" }).addTo(mapState.map);
}
function onMapClick(event) {
  if (mode === "grave") return context?.onGravePoint?.(event.latlng);
  if (!mode) return;
  points.push([event.latlng.lat, event.latlng.lng]); showPreview();
}
...
export function startCemeteryBoundaryDrawing(cemetery, onSaved) {
  start("cemetery", { id: cemetery.id, onSaved });
}
export function startSectionBoundaryDrawing(cemeteryId, section = null, onSaved) {
  start("section", { cemeteryId, id: section?.id || null, name: section?.name || "Yeni bölüm", code: section?.code || null, onSaved });
}
export function startGravePlacement(onGravePoint) {
  start("grave", { onGravePoint });
}
function start(nextMode, nextContext) {
  finishCleanup();
  mapState = window.__regionConsoleMapState || null;
  if (!mapState?.map) return toast(elements, "Harita henüz hazır değil.");
  mode = nextMode; context = nextContext; points = [];
  previousDoubleClickZoom = mapState.map.doubleClickZoom.enabled();
  mapState.map.doubleClickZoom.disable();
  mapState.map.on("click", onMapClick);
  mapState.map.on("dblclick", onDoubleClick);
