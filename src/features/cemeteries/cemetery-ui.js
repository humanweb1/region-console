...
function form(item = null) {
  const neighborhoods = visibleNeighborhoods();
  const selected = item?.region_id || "";
  ...
  elements.dialogBody.querySelector("#cemeteryCancel").onclick = () => closeDialog(elements);
  elements.dialogBody.querySelector("#cemeterySave").onclick = async () => {
    const name = elements.dialogBody.querySelector("#cemeteryName").value.trim();
    const regionId = elements.dialogBody.querySelector("#cemeteryRegion").value;
    ...
    try {
      if (item) await updateCemetery(token(), item.id, { name, code: elements.dialogBody.querySelector("#cemeteryCode").value.trim() || null });
      else await createCemetery(token(), { regionId, name, code: elements.dialogBody.querySelector("#cemeteryCode").value.trim() || null });
      cache = await listCemeteries(token()) || []; loaded = true; closeDialog(elements); toast(elements, item ? "Mezarlık güncellendi." : "Mezarlık oluşturuldu.");
    } catch (error) { toast(elements, error.message); }
  };
}
...
async function openManager() {
  ...
  await load();
  openDialog(elements, "Mezarlıklar", `<div class="cemetery-manager"><div class="dialog-actions"><button id="cemeteryAdd" class="button button-primary" type="button">+ Mezarlık ekle</button></div><div id="cemeteryList" class="cemetery-list"></div></div>`);
  renderList();
  elements.dialogBody.querySelector("#cemeteryAdd").hidden = !hasManage();
  elements.dialogBody.querySelector("#cemeteryAdd").onclick = () => form();
  elements.dialogBody.querySelector("#cemeteryList").onclick = (event) => { const row = event.target.closest("[data-cemetery-id]"); if (!row) return; const item = cache.find((r) => String(r.id) === String(row.dataset.cemeteryId)); if (item && hasManage()) form(item); };
}
