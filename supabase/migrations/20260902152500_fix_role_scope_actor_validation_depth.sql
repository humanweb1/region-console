create or replace function public.admin_save_role(p_role_id uuid,p_name text,p_description text,p_permissions text[] default '{}'::text[],p_scopes jsonb default '[]'::jsonb,p_actor_user_id uuid default null::uuid)
returns jsonb
language plpgsql
security definer
  ...
  for scope in select value from jsonb_array_elements(coalesce(p_scopes,'[]'::jsonb)) loop
    ...
    if p_actor_user_id is not null and not public.is_super_admin_for(p_actor_user_id) then
      if district_uuid is not null then
        if not public.user_has_region_scope_for(p_actor_user_id,district_uuid) then raise exception 'Seçilen ilçe kapsamınız dışında: %',coalesce(scope->>'district_name',scope->>'district_id') using errcode='42501'; end if;
      elsif province_uuid is not null then
        if not public.user_has_region_scope_for(p_actor_user_id,province_uuid) then raise exception 'Seçilen il kapsamınız dışında: %',coalesce(scope->>'province_name',scope->>'province_id') using errcode='42501'; end if;
      elsif country_uuid is not null then
        if not public.user_has_region_scope_for(p_actor_user_id,country_uuid) then raise exception 'Seçilen ülke kapsamınız dışında: %',coalesce(scope->>'country_name',scope->>'country_id') using errcode='42501'; end if;
      end if;
    end if;
  end loop;
